...
def mk_inst_exsum_meetsum(sent, topic):
    return f"""주어진 입력 문장을 아래 <order>에 따라 요약문을 생성해줘.
<order>:
1. 입력 문장은 [num] sentence 와 같이 연결된 회의 대화 내용입니다.
2. 다른 입력으로 [Topic]이 주워지고, 주어진 [Topic]에 적합한 문장을 선택해서 추출 요약을 할겁니다.
3. 전체 대화를 파악하여 [Topic]에 따른 적합한 문장을 선별해주세요. 전체 대화에서 골고루 선별하면 좋습니다.
4. 인삿말이나 추임새, extractive summary와 관련 없는 문장들은 제외하고, 요약에 도움이 되는 문장들만 선별해주세요.
5. 선별된 문장의 id들만 출력하면 되며, 출력 형식은 다음 조건을 따르면 됩니다.
6. 선별한 문장의 id를 다음과 같이 1, 2, ... 나열되는 형식으로 [결과 id 리스트]: 뒤에 출력하세요.
