...
def mk_inst_for_meeting_summary(sent, topic, sum_sample, sum_range="100~400"):
    return ' '.join(f"너는 정말 똑똑한 친구이고 요약을 정말 잘 해. 전체 맥락에서 주요한 내용만 잘 찾아서 요약해야 해. \
                ...
                입력 문단 앞에 [원문]이 주어지고 [요약] 뒤에 너가 요약문을 작성하면 돼. 항상 이 포맷을 잘 지켜주고. \
                이해하지 못한 내용은 None으로 추출해줘. \
                [원문]: {sent} \n [요약]: ".split())

# 3) 다른 요약문 샘플은 [샘플]로 주어지며, 샘플과 같은 문장 스타일로 작성
def mk_inst_for_meeting_summary_new(sent, topic, sum_sample, sum_range="100~400"):
    return f"""너는 정말 똑똑한 친구이고 요약을 정말 잘 해. 전체 맥락에서 주요한 내용만 잘 찾아서 요약해야 해.
주어진 글에 대해서 다음 조건을 지켜줘.
<조건>
1) 요약문의 길이는 {sum_range} 글자 사이로 작성
2) 요약문에 대한 주제는 [주제]로 주어지며, 주제에 적합한 요약문 작성
3) 뉴스 기사처럼 사실적인 문장 스타일로 작성
4) 말투는 존댓말 쓰지말고 "~다."처럼 끝나도록 작성 
5) 요약은 [FEWSHOT] 샘플과 같은 말투로 작성
6) 이해하지 못한 내용은 None으로 출력
...
def mk_inst_for_meeting_summary_v5(sent, topic, sum_sample, sum_range="100~400"):
    return f"""너는 정말 똑똑한 친구이고 요약을 정말 잘 해. 전체 맥락에서 주요한 내용만 잘 찾아서 요약해야 해.
주어진 글에 대해서 다음 조건을 지켜줘.
<조건>
1) 요약문의 길이는 {sum_range} 글자 사이로 작성
2) 요약문에 대한 주제는 [주제]로 주어지며, 주제에 맞는 요약문 작성
3) 뉴스 기사처럼 사실적인 문장 스타일로 작성
4) 말투는 존댓말 쓰지말고, "~다."처럼 끝나도록 입력 원문의 말투 사용해서 작성
5) 이해하지 못한 내용은 None으로 출력
...
#     return ' '.join(f"너는 정말 똑똑한 친구이고 요약을 정말 잘 해. 전체 맥락에서 주요한 내용만 잘 찾아서 요약해야 해. \
#                 주어진 글에 대해서 다음 조건을 지켜줘. \
#                 1) delimiter (구두점 및 구분자 .)로 끊지말고, 2) [원문] 길이에 비례하여 무조건 한 문장으로 요약해줘 길이는 {sum_range} 글자 사이로. 3) 시간 정보(날짜, 요일 등)는 꼭 포함해서, 4) 말투는 존댓말 쓰지말고, 입력 원문의 말투를 가져와. \
#                 입력 문단 앞에 [원문]이 주어지고 [요약] 뒤에 너가 요약문을 작성하면 돼. 항상 이 포맷을 잘 지켜주고. \
#                 이해하지 못한 내용은 None으로 추출해줘. \
#                 [원문]: {sent} \n [요약]: ".split())
...
6. 선별한 문장의 id를 다음과 같이 [1, 2, ...] 나열되는 형식으로 [결과 id 리스트]: 뒤에 리스트 출력하세요.
...
1. 입력 문장은 [num] sentence 와 같이 연결된 회의 대화 내용입니다.
2. [Topic]이 주어지고, 주어진 [Topic]과 관련있는 문장을 모두 찾으시오.
3. 추출 요약으로 자동으로 출력한 문장 [SUM_IDS]가 주어지며, 생성 요약에 도움이 되는 문장을 [SUM_IDS]에서 일부 선별하고, 추가로 [입력 문장]에서 찾아줘.
4. 입력된 문장은 총 {dial_len} 문장입니다. 전체 문장에서 골고루 선별해야 합니다.
5. 문장 수는 {num_ex_sent} ~ {num_ex_sent+10} 만큼 선별하시오.
