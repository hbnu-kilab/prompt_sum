...
def reset_ex_ids(promptor, dialog_dict, ori):
    ...
    for sum_type in ["total_summary", "topic_summary"]:
        ...
        ret_sum_lst = []
        for topic_sum in tqdm(ret_dict[sum_type], total=len(ret_dict[sum_type]), desc="Each sum"):
            ...
            while True:                
                ...
                try:
                    ...
                    aug_ids = eval(tmp_aug)
                    aug_ids = [int(id_el) for id_el in aug_ids]
...
def main():
    ...
    parser.add_argument("-dp", "--data_phases", nargs='+', default=["train", "val", "test"], dest="data_phases", help="--data_types train val test", type=str) 
